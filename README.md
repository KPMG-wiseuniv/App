...
**Permissions에 경우 갤러리에서 이미지를 가지고 오기 위한 저장공간, 카메라 실행을 위한 카메라 권한이 필요한데 현재 어플리케이션은 출시용이 아닌 프로토타입용 이기 때문에<br> apk 설치 파일을 통해 앱을 실행하고자 하는 분들은 앱 실행 전 "저장공간, 카메라" 두가지 권한에 대해 "허용", 혹은 "앱 실행시 허용"을 설정해주시고 실행해주시기 바랍니다.
